...
class FacialExpressionClassifierEng:
    ...
    def xgboost_classifier_participant_based(self):
        ...
        self.plot_pca(au_cr, eng_labels)

        #au_cr, eng_labels = self.feature_selection(au_cr, eng_labels)
        ...
        for i in range(5, 21):
            ...
            train_au_cr, train_eng_labels = [], []
            test_au_cr, test_eng_labels = [], []
            ...
            X_train = train_au_cr
            X_test = test_au_cr
            y_train = train_eng_labels
            y_test = test_eng_labels
            ...
            if (len(y_test) != 0):
                ...
                #model.fit(X_train, y_train)
                #pt = model.predict(X_test)
                #predicted_lr.extend(pt)
                #real_lr.extend(y_test)
        ...
        #print(classification_report(real_lr, predicted_lr))
        print("Other Metrics: \n")
        #print(confusion_matrix(real_lr, predicted_lr))
        #print("Y:", real_lr)
        #print("Predicted:", predicted_lr)
        #accuuracy = accuracy_score(real_lr, predicted_lr)
        print("Accuracy is: ", accuuracy)
    ...
    def knn_classifier_participant_based(self):
        ...
        for number_of_neighbors in range(1,11):
            ...
            for ii in range(len(real)):
                print("{0} - {1}".format(predicted[ii],real[ii]))
            #print(classification_report(real, predicted))
            #print("Other Metrics: \n")
            #print(confusion_matrix(real, predicted))

    def prepare_features_for_engagement(self, data_points):
        eng_labels, au_cr, p_numbers = [], [], []
        for i in range(len(data_points)):
            point = data_points[i]
            # for point in data_points[i]:
            quadrant_label, eng_label, chal_label = self.calculate_labels(point)
            #quadrant_label, eng_label, chal_label = self.calculate_labels_ratio(point)
            if eng_label != "mr":
                    try:
                        tmp = []
                        au_c_avg, au_c_std, au_r_avg, au_r_std = fef.calculate_action_units_parameters(point, 30)
    ...
    def sliding_windows_preparing_features(self):
        ...
        return p_au_cr, p_eng_labels, p_p_numbers
        ...
        for i in range(len(participant_1.data_points)):
            for j in range(len(participant_2.data_points)):
                if participant_1.data_points[i].rate.timestamp == participant_2.data_points[j].rate.timestamp:
                    ...
                    au_cr_1,  eng_label, p_number = self.prepare_features_for_engagement(participant_1.data_points[i])
                    au_cr_2,  eng_label, p_number = self.prepare_features_for_engagement(participant_2.data_points[j])
    ...
    def calculate_labels_multi_level(self, levels, datapoint):
        #print("In set labels...")
        ...
        #           |
        step = 100/levels
        eng_not_set, chal_not_set = False, True
        engagement_label,challenge_label = "mr", "mr"
        for j in range(1, levels):
            i = levels-j
            if chal_not_set and datapoint.rate.challenge > step*i:
                challenge_label = i+1
                chal_not_set = False

            if eng_not_set and datapoint.rate.engagement > step*i:
                engagement_label = i+1
                eng_not_set = False
        quadrant_label = str(engagement_label)+str(challenge_label)
        if 0 < datapoint.rate.engagement < 35:
            engagement_label = 0
        if 35 < datapoint.rate.engagement < 55:
            engagement_label = 1
        if 55< datapoint.rate.engagement:
            engagement_label = 2
        return quadrant_label, engagement_label, challenge_label
    ...
    def calculate_labels_ratio(self,datapoint):
        engagement_label = "2"
        challenge_label = "2"
        quadrant_label = "2"

        if datapoint.rate.engagement < datapoint.rate.challenge-15:
            engagement_label = 1
        elif datapoint.rate.engagement > datapoint.rate.challenge+15:
            engagement_label = 0
        return quadrant_label, engagement_label, challenge_label

    def plot_pca(self, au_cr, labels):
